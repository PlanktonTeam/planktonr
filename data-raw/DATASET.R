...
MapOz <- rnaturalearth::ne_countries(
  scale = "small", country = "Australia",
  returnclass = "sf"
)

# Details for coastal stations
CSCodes <- tibble::tibble(
  StationName = c(
  ...
  StationCode = c(
    "BAH", "SAH", "BAI", "COB", "TOP", "LIP", "DEB", "DES", "DEE", "DEG", "DEK", "DER", "DEU", "LOH",
    "TRM", "RMR", "GNI", "PTD", "CTL", "DBI", "YKK", "FLB", "HOB", "LOR", "GEB", "CHA", "PIB", "CER",
    "WRR", "IRC", "IGB"
  ),
  State = factor(
    c(
      "NSW", "NSW", "NSW", "NSW", "NSW", "NSW", "TAS", "TAS", "TAS", "TAS", "TAS", "TAS", "TAS", "TAS", "GBR",
...
colNRSCode <- data.frame(
  Code = meta_sf$Code,
  Colr = coolor
) %>%
  tibble::deframe()

colNRSName <- data.frame(
  Code = meta_sf$Station,
  Colr = coolor
) %>%
  tibble::deframe()

pchNRSName <- data.frame(
  Code = meta_sf$Station,
  pchr = rep(16, 10)
) %>%
  tibble::deframe()

pchNRSCode <- data.frame(
  Code = meta_sf$Code,
  pchr = rep(16, 10)
) %>%
  tibble::deframe()

ltyNRSCode <- data.frame(
  Code = meta_sf$Code,
  pchr = rep("solid", 10)
) %>%
  tibble::deframe()

ltyNRSName <- data.frame(
  Code = meta_sf$Station,
  pchr = rep("solid", 10)
) %>%
  tibble::deframe()
...
stateLTY <- c(
  "solid", "dashed", "dotted", "dotdash", "longdash", "solid", "dashed", "dotted", "dotdash", "solid", "dashed", "dotted", "dotdash",
...
colCSCode <- data.frame(
  Code = CSCodes$StationCode,
  Colr = stateCol
) %>%
  tibble::deframe()

colCSName <- data.frame(
  Code = CSCodes$StationName,
  Colr = stateCol
) %>%
  tibble::deframe()

pchCSCode <- data.frame(
  Code = CSCodes$StationCode,
  Colr = statePCH
) %>%
  tibble::deframe()

pchCSName <- data.frame(
  Code = CSCodes$StationName,
  Colr = statePCH
) %>%
  tibble::deframe()

ltyCSCode <- data.frame(
  Code = CSCodes$StationCode,
  pchr = stateLTY
) %>%
  tibble::deframe()

ltyCSName <- data.frame(
  Code = CSCodes$StationName,
  pchr = stateLTY
) %>%
  tibble::deframe()
