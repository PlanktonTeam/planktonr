...
pr_get_FuncGroups <- function(Survey = "NRS", Type = "Z", ...) {
  ...
  if (Type == "P") {
    var_names <- c(
      "Centric diatom", "Ciliate", "Cyanobacteria", "Dinoflagellate", "Flagellate", "Foraminifera",
      "Pennate diatom", "Radiozoa", "Silicoflagellate"
    )
  } else if (Type == "Z") {
    var_names <- c(
      "Ascidian", "Ciliate", "Ciliophora", "Coelenterate", "Ctenophore", "Echinoderm", "Euphausiid",
  ...
  if (Survey == "CPR") {
    ...
    df <- df %>%
      dplyr::select(
        "StationName", "StationCode", "SampleTime_Local",
  ...
  if (Type == "P") {
    df <- df %>%
      dplyr::mutate(
        Parameters = ifelse(.data$Parameters %in% c("Centric diatom", "Pennate diatom", "Dinoflagellate", "Cyanobacteria"), .data$Parameters, "Other"),
        Parameters = factor(.data$Parameters, levels = c(
          "Centric diatom", "Pennate diatom", "Dinoflagellate", "Cyanobacteria",
          "Other"
        ))
      )
  } else if (Type == "Z") {
    df <- df %>%
      dplyr::mutate(
        Parameters = ifelse(.data$Parameters %in% c("Copepod", "Appendicularian", "Mollusc", "Cladoceran", "Chaetognath", "Thaliacean"), .data$Parameters, "Other"),
        Parameters = factor(.data$Parameters, levels = c(
          "Copepod", "Appendicularian", "Mollusc", "Cladoceran", "Chaetognath", "Thaliacean",
          "Other"
        ))
      )
  }
  ...
  df <- df %>%
    ...
    dplyr::summarise(
      Values = sum(.data$Values, na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
pr_get_PCIData <- function() {
  dat <- planktonr::pr_get_Indices("CPR", "W", near_dist_km = 250) %>%
    ...
    dplyr::mutate(
      Longitude = round(.data$Longitude, 0),
    ...
    dplyr::summarise(
      PCI = mean(.data$Values, na.rm = TRUE),
      .groups = "drop"
    )
}
...
pr_get_CTI <- function(Type = "Z") {
  ...
  dat <- dat %>%
    ...
    dplyr::summarise(
      CTI = sum(.data$Abundance * .data$STI, na.rm = TRUE) / sum(.data$Abundance, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    as.data.frame()
...
pr_get_DayNight <- function(Type = "Z") {
  ...
  daynight_df <- suncalc::getSunlightTimes(
    data = dates, # TODO quicker to change to Local now that it exists.
  ...
  dat <- dat %>%
    ...
    dplyr::summarise(
      Species_m3 = mean(.data$Species_m3, na.rm = TRUE),
      .groups = "drop"
    )
}
...
#' @examples
#' df <- data.frame(
#'   TaxonGroup = c("Dinoflagellate", "Cyanobacteria"),
...
pr_add_Carbon <- function(df, meth) {
  if (meth %in% "CPR") {
    df <- df %>%
      dplyr::mutate(
        BV_Cell = .data$BioVolume_um3m3 / .data$PhytoAbund_m3, # biovolume of one cell
        Carbon = dplyr::case_when(
          .data$TaxonGroup == "Dinoflagellate" ~ 0.76 * (.data$BV_Cell)^0.819, # conversion to Carbon based on taxongroup and biovolume of cell
          .data$TaxonGroup == "Ciliate" ~ 0.22 * (.data$BV_Cell)^0.939,
          .data$TaxonGroup == "Cyanobacteria" ~ 0.2,
          TRUE ~ 0.288 * (.data$BV_Cell)^0.811
        ),
        Carbon_m3 = .data$PhytoAbund_m3 * .data$Carbon
      ) # Carbon per m3
    return(df)
  }
  ...
  if (meth %in% "NRS") {
    df <- df %>%
      dplyr::mutate(
        BV_Cell = .data$Biovolume_um3L / .data$Cells_L, # biovolume of one cell
        Carbon = dplyr::case_when(
          .data$TaxonGroup == "Dinoflagellate" ~ 0.76 * (.data$BV_Cell)^0.819, # conversion to Carbon based on taxongroup and biovolume of cell
          .data$TaxonGroup == "Ciliate" ~ 0.22 * (.data$BV_Cell)^0.939,
          .data$TaxonGroup == "Cyanobacteria" ~ 0.2,
        ...
        Carbon_L = .data$Cells_L * .data$Carbon
      ) # Carbon per litre
    return(df)
  }
}
