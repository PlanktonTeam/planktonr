...
pr_plot_Trends <- function(df, Trend = "Raw", Survey = "NRS", method = "lm", trans = "identity") {
  ...
  if (Trend %in% c("Year_Local", "Month_Local")) {
    df <- df %>%
      ...
      dplyr::summarise(
        value = mean(.data$Values, na.rm = TRUE),
        N = dplyr::n(),
...
pr_plot_Climatology <- function(df, Survey = "NRS", Trend = "Month", trans = "identity") {
  ...
  p1 <- ggplot2::ggplot(df_climate, ggplot2::aes(
    x = !!Trend, y = .data$mean, fill = .data$StationName,
...
pr_plot_EOVs <- function(df, EOV = "Biomass_mgm3", Survey = "NRS", trans = "identity", col = "blue", labels = TRUE) {
  ...
  if (Survey == "LTM") {
    p1 <- p1 + ggplot2::scale_x_datetime(
  ...
  p3 <- ggplot2::ggplot(df) +
    ...
    ggplot2::geom_smooth(ggplot2::aes(x = .data$Month, y = .data$Values),
      method = "loess",
      formula = "y ~ x", colour = col, fill = col, alpha = 0.5
    ) +
    ggplot2::scale_y_continuous(trans = trans, expand = ggplot2::expansion(mult = c(0.02, 0.02))) +
    ggplot2::scale_x_continuous(
      breaks = seq(0.5, 6.3, length.out = 12), expand = ggplot2::expansion(mult = c(0.02, 0.02)),
      labels = c("J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D")
    ) +
    ggplot2::xlab("Month") +
    theme_pr() +
...
pr_plot_DayNight <- function(df) {
  ...
  plots <- ggplot2::ggplot(df, ggplot2::aes(.data$Month_Local, .data$Species_m3)) +
    ...
    ggplot2::scale_x_continuous(
      breaks = seq(1, 12, length.out = 12), expand = c(0, 0),
      labels = c("J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D")
    ) +
    theme_pr() +
...
#' df <- df %>% dplyr::filter(
#'   Parameters == "Archaea_unique_ASVs",
#'   SampleDepth_m < 101
#' )
...
pr_plot_latitude <- function(df, Fill_NA = FALSE, maxGap = 3) {
  ...
  interp_vals <- pracma::interp2(
    y = seq(0, max(df2$SampleDepth_m, na.rm = TRUE), length.out = nrow(mat)),
