...
pr_get_EOVs <- function(Survey = "NRS", ...) {
  if (Survey == "CPR") {
    var_names <- c(
      "BiomassIndex_mgm3", "PhytoBiomassCarbon_pgm3",
    ...
    Pol <- Polr %>%
      dplyr::select(
        tidyselect::starts_with(c("SampleTime_Local", "Year_Local", "Month_Local", "BioRegion", "DistanceFromBioregion_m")),
    ...
    Pol <- Pol %>%
      ...
      dplyr::mutate(
        anomaly = (.data$Values - means) / sd,
        Survey = "CPR"
      ) %>%
      planktonr::pr_reorder()
  } else if (Survey == "NRS") {
    var_names <- c(
      "Biomass_mgm3", "PhytoBiomassCarbon_pgL",
    ...
    Pol <- Polr %>%
      dplyr::select(
        "SampleTime_Local", "Year_Local", "Month_Local", "StationName",
    ...
    LTnuts <- pr_get_LTnuts() %>%
      ...
      dplyr::group_by(
        .data$StationName, .data$StationCode, .data$SampleTime_Local, .data$Month_Local,
    ...
    Pol <- LTnuts %>%
      dplyr::left_join(means, by = c("StationName", "Parameters")) %>%
      dplyr::mutate(
        anomaly = (.data$Values - .data$means) / .data$sd,
        Survey = "LTM"
      ) %>%
      planktonr::pr_reorder()
  }
...
pr_get_PolicyInfo <- function(Survey = "NRS", ...) {
  if (Survey == "NRS") {
    NRSinfo <- pr_get_NRSStation() %>%
      dplyr::mutate(
        Region = dplyr::case_when(
          .data$StationCode %in% c("DAR") ~ "Tropical North",
        ...
        Features = dplyr::case_when(
          .data$StationCode %in% c("DAR") ~ "broad, shallow shelf seas with strong tidal influence and tropical neritic communities.",
        ...
        now = dplyr::case_when(
          .data$StationCode %in% c("DAR", "YON", "NSI", "PHB", "MAI", "KAI", "ROT") ~ "and is ongoing",
    ...
    CPRinfo <- pr_get_CPRTrips() %>%
      ...
      dplyr::summarise(
        SampleStartDate = as.Date(min(.data$SampleTime_UTC)),
        Miles = dplyr::n() * 4 * 5
      ) %>%
      dplyr::mutate(Features = dplyr::case_when(
        .data$BioRegion %in% c("South-east") ~ "narrow shelf intensifying currents, eddies and upwellings with low nutrient and primary productivity",
