...
pr_get_CTD <- function(){
  ...
  rawCTD <- readr::read_csv(paste0(pr_get_site(), "IMOS_-_Australian_National_Mooring_Network_(ANMN)_-_CTD_Profiles.csv"), na = "", skip = 29,
    ...
    pr_rename() %>%
    rename(SampleDepth_m = DEPTH, Salinity_psu = PSAL, Salinity_flag = PSAL_quality_control,
                  SampleDateUTC = time_coverage_start, StationCode = site_code,  Temperature_degC = TEMP) %>% # Can't rename this in pr_rename due to replicate name
    filter(grepl("NRS", StationCode)) %>% # Subset to NRS only
    mutate(TripCode = ifelse(StationCode == 'NRSDAR', paste0(substr(StationCode,4,6), format(SampleDateUTC, "%Y%m%d_%H:%M")),
                                    paste0(substr(StationCode,4,6), format(SampleDateUTC, "%Y%m%d"))),
                  Chla_mgm3 = ifelse(!is.na(Chla_mgm3), Chla_mgm3, CHLF)) %>%
    pr_get_StationName() %>%
    select(file_id, StationName, TripCode, SampleDateUTC, Latitude, Longitude,
    ...
    mutate(tz = lutz::tz_lookup_coords(Latitude, Longitude, method = "fast", warn = FALSE),
  ...
  NRSSamp <- pr_get_NRSTrips() %>%
    filter(stringr::str_detect(TripCode, "PH4", negate = TRUE))

  Stations <- rawCTD %>%
    select(TripCode) %>%
    mutate(stations = as.factor(substr(TripCode, 1, 3))) %>%
    select(stations) %>%
    distinct()
  ...
  for (y in 1:nlevels(Stations$stations)){
    ...
    rawCTDCast <- rawCTD %>%
      select(file_id, SampleDateUTC, TripCode) %>%
      filter(substr(TripCode, 1, 3) == station) %>%
      distinct()
