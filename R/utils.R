...
#'
#' Internal function to load the loaction of the raw plankton data files.
#' @return A string with location of raw plankton data
#' @export
#' @examples
#' NRSStation <- get_NRSStation()
#' @importFrom magrittr "%>%"
...
getNRSTrips <- function(){
  NRSSamp <- readr::read_csv(paste0(get_raw_plankton, "BGC_Trip.csv"), na = "") %>%
    ...
    dplyr::filter(PROJECTNAME == "NRS") %>%
    dplyr::mutate(Year = year(SampleDateLocal),
...
#   NRSZdat <- readr::read_csv(paste0(raw, "BGC_Zoop_Raw.csv"), na = "") %>%
...
#   NRSZcount <- readr::read_csv(paste0(raw, "BGC_Zoop_CountRaw.csv"), na = "") %>%
...
#   NRSZcl <- readr::read_csv(paste0(raw, "BGC_Zoop_ChangeLog.csv"), na = "") %>%
#     dplyr::rename(TaxonName = TAXON_NAME, StartDate = START_DATE, ParentName = PARENT_NAME)
#   return(NRSZcl)
# }
#
...
#   ZInfo <- readr::read_csv(paste0(raw, "taxon_info.csv"), na = "",
...
#   chemistry <- readr::read_csv(paste0(raw, "BGC_Chemistry.csv"), na = c("", NaN),
...
# getPico <- function(){
#   Pico <- readr::read_csv(paste0(raw, "BGC_Picoplankton.csv"), na = "") %>%
#     dplyr::rename(TripCode = TRIP_CODE, SampleDepth_m = SAMPLEDEPTH_M, Replicate = REPLICATE, SampleDate_Local = SAMPLEDATE_LOCAL,
...
# getCTD <- function(){
#   rawCTD <- readr::read_csv(paste0(raw, "IMOS_-_Australian_National_Mooring_Network_(ANMN)_-_CTD_Profiles.csv"), na = "", skip = 29,
...
#     dplyr::mutate(TripCode = ifelse(site_code == 'NRSDAR', paste0(site_code, format(time_coverage_start, "%Y%m%d_%H:%M")),
...
#     dplyr::mutate(tz = paste("\"", tz_lookup_coords(Latitude, Longitude, method = "fast"),"\""),
#            castTime_Local = with_tz(CastTime_UTC, tz = "Australia/Perth")) %>%
#     dplyr::filter(!file_id %in% c(2117, 2184, 2186, 2187))
#
#   NRSSamp <- getNRSTrips() %>%
# dplyr::filter(!grepl('PH4', TripCode))
#
...
#
# # getCTD <- function(){
# #   CTD <- readr::read_csv("https://raw.githubusercontent.com/PlanktonTeam/IMOS_Toolbox/master/Plankton/RawData/nrs_ctd.csv", na = "",
...
#   CPRTrips <- readr::read_csv(paste0(raw, "CPR_Samp.csv"), na = "(null)") %>%
...
#   CPRZooCount <- readr::read_csv(paste0(raw, "CPR_Zoop_CountRaw.csv"), na = "(null)") %>%
