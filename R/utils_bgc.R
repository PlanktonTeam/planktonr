...
pr_get_NRSChemistry <- function() {
  var_names <- c(
    "SecchiDepth_m", "Silicate_umolL", "Phosphate_umolL", "Ammonium_umolL", "Nitrate_umolL", "Nitrite_umolL",
    "Oxygen_umolL", "DIC_umolkg", "Alkalinity_umolkg", "Salinity",
    "NOx_umolL", "DIN_umolL", "Redfield"
  ...
  dat <- pr_get_Raw(file) %>%
    ...
    dplyr::select(
      "Project", "SampleTime_Local", "Month_Local", "SampleDepth_m", "TripCode",
...
pr_get_NRSPigments <- function(Format = "all") {
  ...
  if (Format == "binned") {
    ...
    var_names <- c(
      "Allo_mgm3", "AlphaBetaCar_mgm3", "Anth_mgm3", "Asta_mgm3", "BetaBetaCar_mgm3", "BetaEpiCar_mgm3", "Butfuco_mgm3",
  ...
  dat <- dat %>%
    dplyr::select(
      "Project", "TripCode", "SampleTime_Local", "Month_Local", "SampleDepth_m", "StationName", "StationCode",
...
pr_get_NRSPico <- function() {
  ...
  dat <- pr_get_Raw(file) %>%
    ...
    dplyr::select(
      "Project", "TripCode", "SampleTime_Local", "Month_Local", "Year_Local",
...
pr_get_NRSMicro <- function(Survey = "NRS") {
  var_names <- c(
    "Prochlorococcus_cellsmL", "Synechococcus_cellsmL", "Picoeukaryotes_cellsmL",
  ...
  if (Survey == "Coastal") {
    ...
    dat <- readr::read_csv("https://raw.githubusercontent.com/AusMicrobiome/microbial_ocean_atlas/main/data/oceanViz_AM_data.csv") %>%
      pr_rename() %>%
      dplyr::mutate(
        sample = as.numeric(stringr::str_sub(.data$code, -5, -1)),
...
pr_get_CSChem <- function() {
  var_names <- c(
    "Silicate_umolL", "Nitrate_umolL", "Phosphate_umolL", "Ammonium_umolL", "Chla_mgm3", "Temperature_degC",
    "Salinity_psu", "Oxygen_umolL", "Turbidity_NTU", "Density_kgm3"
  )

  CoastalStations <- c(
    "Balls Head", "Salmon Haul", "Bare Island", "Cobblers Beach", "Towra Point", "Lilli Pilli",
    "Derwent Estuary B1", "Derwent Estuary B3", "Derwent Estuary E", "Derwent Estuary G2",
...
pr_get_NRSCTD <- function() {
  ...
  ctd_vars <- c(
    "Project", "file_id", "StationName", "StationCode", "TripCode",
...
pr_get_LTnuts <- function() {
  var_names <- c(
    "Silicate_umolL", "Phosphate_umolL", "Ammonium_umolL", "Nitrate_umolL", "Nitrite_umolL",
  ...
  NutsLT <- readr::read_csv(system.file("extdata", "LongTermMonitoringData.csv", package = "planktonr", mustWork = TRUE),
    ...
    col_types = readr::cols(
      NITRITE_VALUE = readr::col_number(),
    ...
    dplyr::mutate(
      StationCode = gsub(".*[-]([^.]+)[-].*", "\\1", .data$SURVEY_NAME),
      Project = "LTM",
    ...
    pr_rename() %>%
    dplyr::rename(
      SampleTime_Local = "START_TIME",
      SampleDepth_m = "PRESSURE"
    ) %>%
    dplyr::select(
      "StationCode", "Project", "SampleTime_Local", "Month_Local", "Year_Local",
  ...
  Nuts <- pr_get_NRSChemistry() %>%
    dplyr::filter(
      .data$StationCode %in% c("MAI", "ROT", "PHB"),
  ...
  CTD <- pr_get_NRSCTD() %>%
    dplyr::select(
      "Project", "StationCode", "StationName", "Month_Local", "Year_Local",
