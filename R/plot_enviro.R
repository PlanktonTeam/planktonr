...
#'
#' @examples
#' df <- pr_get_NRSChemistry() %>% dplyr::filter(
...
pr_plot_Enviro <- function(df, Trend = "None", trans = "identity") {
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(.data$SampleTime_Local, .data$Values,
    colour = .data$StationName,
  ...
  mdat <- df %>%
    ...
    dplyr::summarise(
      MonValues = mean(.data$Values, na.rm = TRUE),
  ...
  m <- ggplot2::ggplot(mdat, ggplot2::aes(.data$Month_Local, .data$MonValues,
    colour = .data$StationName,
    fill = .data$StationName, linetype = .data$StationName
  )) +
    ggplot2::geom_point() +
    ...
    ggplot2::scale_x_continuous(
      breaks = seq(1, 12, length.out = 12), expand = ggplot2::expansion(mult = c(0.02, 0.02)),
...
#'
#' @examples
#' df <- pr_get_NRSEnvContour("Chemistry") %>% dplyr::filter(
#'   Parameters == "Nitrate_umolL",
...
pr_plot_NRSEnvContour <- function(df, Interpolation = TRUE, Fill_NA = FALSE, maxGap = 3) {
  ...
  df <- df %>%
    dplyr::mutate(
      ...
      SampleDepth_m = dplyr::case_when(
        .data$StationName %in% c("Darwin") ~ round(.data$SampleDepth_m / 10, 0) * 10,
  ...
  if (Interpolation == FALSE) {
    ...
    plotfunc <- function(stations) {
      ...
      interped <- expand.grid(
        SampleDepth_m = Depths,
        MonthSince = Months2
      )

      interp_vals <- pracma::interp2(
        y = seq(0, max(df$SampleDepth_m, na.rm = TRUE), length.out = nrow(mat)),
      ...
      dfInterp <- dplyr::bind_cols(interped, Values = interp_vals) %>%
        data.frame() %>%
        dplyr::mutate(
