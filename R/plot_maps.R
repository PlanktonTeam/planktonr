...
pr_plot_PCI <- function(df) {
  cprmap <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_gradient(
      low = "light green", high = "darkgreen",
      breaks = c(0, 1, 2, 3),
...
#' @examples
#' df <- data.frame(
#'   Longitude = c(110, 130, 155, 150), Latitude = c(-10, -35, -27, -45),
...
pr_plot_FreqMap <- function(df, species, interactive = TRUE) {
  dfa <- df %>%
    dplyr::select("Season", "Latitude", "Longitude", "Survey") %>%
    dplyr::distinct()
  ...
  if (interactive == FALSE) {
    ...
    plotlist <- function(dflist) {
      ...
      fmap <- leaflet::leaflet() %>%
        ...
        leaflet::addPolygons(
          data = mbr, group = "Marine Bioregions",
          color = ~Colour, fill = ~Colour,
          opacity = 1, fillOpacity = 0.3,
          weight = 1
        ) %>%
        leaflet::addCircleMarkers(
          data = dfa, group = "All Samples",
        ...
        leaflet::addCircleMarkers(
          data = dfCPR, group = "Continuous Plankton Recorder",
          lat = ~Latitude, lng = ~Longitude,
          radius = 5,
        ...
        leaflet::addCircleMarkers(
          data = dfNRS, group = "National Reference Stations",
          lat = ~Latitude, lng = ~Longitude,
  ...
  if (interactive == TRUE) {
    ...
    if (labels == TRUE) {
      labs_cpr <- lapply(seq(nrow(df_CPR)), function(i) {
        paste(
          "<strong>Sample Date:</strong>", df_CPR$SampleTime_Local[i], "<br>",
      ...
      labs_mbr <- lapply(seq(nrow(mbr)), function(i) {
        ...
        sapply(
          strwrap(
            paste(
              "<strong>The ", mbr$REGION[i], br, "</strong>",
    ...
    if (is.data.frame(df) == FALSE) {
      labs_nrs <- lapply(seq(nrow(df_NRS)), function(i) {
        ...
        paste(
          "<strong>National Reference Station:</strong>", df_NRS$Name[i], "<br>",
    ...
    map <- leaflet::leaflet() %>%
      ...
      leaflegend::addLegendFactor(
        pal = leaflet::colorFactor("#FFA500", "National Reference Stations"),
    ...
    MapOz <- rnaturalearth::ne_countries(
      scale = "medium", country = "Australia",
      returnclass = "sf"
    )
    ...
    if ("CPR" %in% Survey$Survey) {
      gg <- gg +
        ggplot2::geom_sf(
          data = PMapData2 %>% dplyr::filter(.data$Survey == "CPR"),
    ...
    if ("NRS" %in% Survey$Survey) {
      gg <- gg +
        ...
        ggplot2::geom_sf_text(
          data = PMapSum %>% dplyr::filter(.data$Survey == "NRS"),
