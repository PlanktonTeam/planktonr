...
pr_get_indices("NRS", "Z") %>% 
  filter(StationName %in% c("Yongala", "Port Hacking", "Maria Island") & 
         parameters == "ZoopAbundance_m3") %>% 
  droplevels() %>% 
  pr_plot_tsclimate(Survey = "NRS", Scale = "log10")
```
...
pr_plot_trends(datNRS_zoo, Trend = "Raw", Survey = "NRS", Method = "lm", Scale = "log10")
```

Now lets check the seasonal cycle.
```{r,include=TRUE}
pr_plot_trends(datNRS_zoo, Trend = "Month", Survey = "NRS", Method = "loess", Scale = "log10")
```
