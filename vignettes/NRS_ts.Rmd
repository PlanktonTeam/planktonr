...
vignette: >
  %\VignetteIndexEntry{Analysing NRS timeseries}
...
```{r,include=TRUE}
datNRS_zoo <- pr_get_Indices("NRS", "Z")
```
...
```{r,include=TRUE}
pr_plot_tsclimate(datNRS_zoo, Survey = "NRS", trans = "log10")
```
...
```{r,include=TRUE}
pr_get_Indices("NRS", "Z") %>% 
  filter(StationName %in% c("Yongala", "Port Hacking", "Maria Island") & 
         Parameters == "ZoopAbundance_m3") %>% 
  droplevels() %>% 
  pr_plot_tsclimate(Survey = "NRS", trans = "log10")
```
...
```{r,include=TRUE}
pr_plot_tsclimate(datSize, Survey = "NRS")
```
...
pr_plot_Trends(datNRS_zoo, Trend = "Raw", Survey = "NRS", method = "lm", trans = "log10")
```

Now lets check the seasonal cycle.
```{r,include=TRUE}
pr_plot_Trends(datNRS_zoo, Trend = "Month", Survey = "NRS", method = "loess", trans = "log10")
```
...
```{r,include=TRUE}
NRSfgz <- pr_get_FuncGroups(Survey = "NRS", Type = "Z") %>% 
  filter(StationName %in% c("Yongala", "Maria Island")) %>% 
  droplevels()
