testthat::test_that("Correct function output", {
  ...
  testthat::expect_equal(class(planktonr::pr_get_EOVs("NRS") %>%
    dplyr::filter(
      .data$Parameters != "Oxygen_umolL",
      !.data$StationCode %in% c("NIN", "ESP")
    ) %>%
    pr_get_Coeffs() %>%
    pr_plot_EOVs(
      EOV = "Biomass_mgm3", Survey = "NRS", trans = "identity",
      labels = "yes"
    ))[1], "patchwork")
  ...
  testthat::expect_equal(class(planktonr::pr_get_EOVs("CPR") %>%
    pr_get_Coeffs() %>%
    pr_plot_EOVs(
      EOV = "Biomass_mgm3", Survey = "CPR", trans = "identity",
      labels = "no"
    ))[1], "patchwork")
  ...
  testthat::expect_equal(class(planktonr::pr_get_EOVs("LTM") %>%
    pr_get_Coeffs() %>%
    pr_plot_EOVs(
      EOV = "Biomass_mgm3", Survey = "LTM", trans = "identity",
      labels = "no"
    ))[1], "patchwork")
  ...
  testthat::expect_equal(class(data.frame(
    Month = rep(seq(1, 12, 1), 2),
  ...
  testthat::expect_equal(class(pr_get_NRSEnvContour("Pico") %>%
    dplyr::filter(
      Parameters == "Prochlorococcus_cellsmL",
  ...
  testthat::expect_equal(class(
    pr_get_NRSMicro("GO-SHIP") %>%
      ...
      pr_plot_latitude(Fill_NA = TRUE, maxGap = 5)
  )[1], "patchwork")
})
